    ...
    for _, citing_paper in main_df.iterrows():
        ...
        for cited_title_norm in known_normalized_titles:
            ...
            if cited_title_norm in references_norm:
                ...
                cited_by_map[cited_title_norm].append(title_map[citing_title_norm]['original_title'])
    
    print(f"找到了 {len(citation_edges)} 条引用关系。")
    return title_map, citation_edges, in_degree_counts, out_degree_counts, cited_by_map


def generate_interactive_graphs(threshold_pairs, use_and_logic, exclude_isolated_nodes, title_map, citation_edges, in_degree_counts, out_degree_counts, output_folder):
    """根据一系列参数，生成美化后的、可点击节点的交互式网络图。"""
    # ... (此函数无需修改) ...
    print("\n--- 步骤 3: 生成交互式网络图 ---")
    ...
    for out_thresh, in_thresh in threshold_pairs:
        ...
        net = Network(height='100vh', width='100%', bgcolor='#111827', font_color='#d1d5db', directed=True)
        ...
        for norm_title, info in title_map.items():
            ...
            is_isolated = (in_d == 0 and out_d == 0)
            if exclude_isolated_nodes and is_isolated:
                continue
            ...
            if passes_degree_filter:
                ...
                hover_title = (f"<div style='padding:5px; font-family:sans-serif; background-color:#374151; color:white; border-radius:3px;'>"
                             f"<strong>{html.escape(info['original_title'])}</strong><br>"
                             f"Year: {info.get('year', 'N/A')}<br>"
                             f"DOI: {doi or 'N/A'}<br>"
                             f"<hr style='border-color:#6b7280; margin: 3px 0;'>"
                             f"Cited By (In): {in_d}<br>"
                             f"Cites (Out): {out_d}</div>")
                
                node_size = 12 + in_d * 1.5
                ...
                nodes_in_current_graph.add(norm_title)
        
        if not nodes_in_current_graph:
            print(f"在此条件下，没有文献满足要求，跳过生成此图。")
            continue

        print(f"找到 {len(nodes_in_current_graph)} 个满足条件的节点。")
        for edge in citation_edges:
            if edge['source'] in nodes_in_current_graph and edge['target'] in nodes_in_current_graph:
        ...
        with open(output_path, 'r+', encoding='utf-8') as f:
            content = f.read()
            f.seek(0, 0)
            title_html = (f"<h1 style='font-family: sans-serif; color: #f3f4f6; text-align: center; margin-top: 20px;'>Citation Network</h1>"
                          f"<p style='font-family: sans-serif; color: #9ca3af; text-align: center; margin-bottom: 20px;'>"
                          f"Filter: Out-Degree ≥ {out_thresh} {logic_str} In-Degree ≥ {in_thresh}</p>")
            js_click_handler = """<script type="text/javascript">document.addEventListener("DOMContentLoaded", function() { if (network) { network.on("click", function (params) { if (params.nodes.length > 0) { var nodeId = params.nodes[0]; var node = nodes.get(nodeId); if (node.doi_url) { window.open(node.doi_url, '_blank'); } } }); } });</script>"""
            final_html = content.replace('<body>', f'<body>{title_html}').replace('</body>', js_click_handler + '</body>')
            f.write(final_html)
        
        print(f"美化后的网络图已保存到: {output_path}")


def generate_summary_csv(title_map, in_degree_counts, out_degree_counts, cited_by_map, output_folder):
    """生成包含所有分析结果的最终CSV摘要文件到输出文件夹"""
    # ... (此函数无需修改) ...
    print("\n--- 步骤 4: 生成摘要CSV文件 ---")
    table_data = []
    for norm_title, info in title_map.items():
        table_data.append({
            ...
            'Cited_By': "; ".join(cited_by_map.get(norm_title, []))
        })
...
def main():
    ...
    try:
        ...
        if not os.path.isdir(DATA_FOLDER):
            raise FileNotFoundError(f"输入文件夹不存在: '{DATA_FOLDER}'. 请检查 .env 文件中的 ANALYSIS_BASE_FOLDER 设置是否正确，并确保该路径下有 'citations' 子文件夹。")
        ...
        title_map, edges, in_degrees, out_degrees, cited_by_map = build_citation_network(main_df)
        ...
        generate_interactive_graphs(
            ...
            in_degree_counts=in_degrees,
            out_degree_counts=out_degrees,
            output_folder=OUTPUT_FOLDER
        )
        
        generate_summary_csv(title_map, in_degrees, out_degrees, cited_by_map, OUTPUT_FOLDER)
